...
from cal_ctr import cal_ctr_net
...
from utils.cal_ctr_dataset import CalCtrDataset
...
from arguments import get_parser, update_dir_train, update_dir_test, update_dir_resume, update_dir_match, update_dir_cal_ctr
...
def train_semi_net(net,
    ...
    if args.resume == True:
        ...
        # val_loader = DataLoader(val, batch_size=1, shuffle=False, pin_memory=True, drop_last=False, num_workers=8)
        # valid_score = eval_net(args, net, val_loader, device)
        # writer.add_scalar('valid', valid_score, global_step)
        # # save checkpoint
        # checkpoint_best = {
...
if __name__ == '__main__':
    ...
    try:
        if (not args.test) and (not args.match) and (not args.cal_ctr):
            if args.semi == True:
            ...
            cal_ctr_dataset = CalCtrDataset(args)
            cal_ctr_loader = DataLoader(cal_ctr_dataset, batch_size=1, shuffle=False,  pin_memory=True, drop_last=False)
            cal_ctr_net(args, hog, cal_ctr_loader, device)
        else:
