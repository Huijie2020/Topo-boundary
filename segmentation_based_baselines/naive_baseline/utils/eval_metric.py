...
def thr_eval(name_in):
    ...
    with tqdm(total=len(img_list), unit='img') as pbar:
        for i,img in enumerate(img_list):
            ...
            # else:
            #     if args.tta == True:
            #         pre_image_conf = pre_image/255
            #         pre_image_conf_mask = (pre_image_conf >= args.tta_thr)
            #         pre_image = pre_image_conf * pre_image_conf_mask
            #         # tta_thr = args.tta_thr * np.max(pre_image)
            #         # thr_mask = (pre_image >= tta_thr)
            #         # pre_image = pre_image * thr_mask
            #     else:
            #         pre_image_conf = pre_image / 255
            #         pre_image_conf_mask = (pre_image_conf >= args.no_tta_thr)
            #         pre_image = pre_image_conf * pre_image_conf_mask
            #         # no_tta_thr = args.no_tta_thr * np.max(pre_image)
            #         # thr_mask = (pre_image >= no_tta_thr)
            #         # pre_image = pre_image * thr_mask
