...
class GrowDataset(Dataset):
    def __init__(self, args, valid=False):
        ...
        with open(
                '/mnt/git/Topo-boundary/conn_experiment/dataset_split/1_data_split/data_split_1data_99unsup_100val_431test.json','r') as sup_file:
    ...
    # random crop image and gt
    def crop_grow(self, image, size, overlap_location):
        w, h = image.size
        newW, newH = int(w), int(h)
        assert newW > 0 and newH > 0, 'Scale is too small'
        image = image.resize((newW, newH))
        ...
        start_x = overlap_location[1]
        start_y = overlap_location[0]
    ...
    def __getitem__(self, i):
        ...
        load_file_name = idx["file_name"]
        if load_file_name in self.sup20:
            file_name = load_file_name
            img_file = os.path.join(self.imgs_dir, file_name + '.png')
            mask_file = os.path.join(self.masks_dir, load_file_name + '.png')
        else:
            file_name = load_file_name
            for i in range(len(load_file_name)):
                if file_name[-1] != "_":
            ...
            img_file = os.path.join(self.imgs_dir, file_name + '.png')
            mask_file = os.path.join(self.masks_dir, load_file_name + '.png')
            overlap_location = idx["overlap_ul"]
        ...
        if (not self.test) and (not self.valid):
            if load_file_name in self.sup20:
                ...
                if self.data_augumentation:
                    img = self.crop_grow(img, self.crop_size, overlap_location)
                    img, mask = self.data_augu(img, mask)
                else:
                    img = self.crop_grow(img, self.crop_size, overlap_location)
                    img = np.array(img)
